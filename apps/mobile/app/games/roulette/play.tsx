        ...
        {winner ? (
          <>
            <Animated.View entering={FadeInDown.delay(80).springify().damping(18).stiffness(140)}>
              <TouchableOpacity style={[styles.primaryBtn, { backgroundColor: colors.accentBright }]} onPress={spin} activeOpacity={0.85}>
                <View style={styles.relancerContent}>
                  ...
                  <Text style={styles.primaryBtnText}>Relancer</Text>
                </View>
              </TouchableOpacity>
            </Animated.View>
            ...
            <Animated.View entering={FadeIn.duration(300)} style={styles.resultWrap}>
              ...
              >
                <Text style={[styles.resultLabel, { color: colors.onDarkTertiary }]}>À la caisse</Text>
                <Text style={[styles.resultName, { color: colors.onDarkPrimary }]} numberOfLines={1}>{winner.name}</Text>
                <Text style={[styles.resultSub, { color: colors.onDarkSecondary }]}>paie l’addition 🍻</Text>
              </LinearGradient>
            </Animated.View>
          </>
        ) : (
          <Animated.View entering={FadeInDown.delay(80).springify().damping(18).stiffness(140)}>
            <TouchableOpacity
              ...
              activeOpacity={0.85}
            >
              <Text style={styles.primaryBtnText}>{spinning ? 'Ça tourne…' : 'Tourner la roue'}</Text>
            </TouchableOpacity>
          </Animated.View>
        )}
      </View>
