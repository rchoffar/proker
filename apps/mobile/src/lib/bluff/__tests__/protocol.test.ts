...
describe('redactFor — the leak test', () => {
  it('never ships another player’s cards nor the board stock before the reveal', () => {
    let state = dealtGame();
    const phases: BluffState[] = [state];
    state = reduce(state, { type: 'chooseBoard', playerId: state.starterId, boardCount: 2 });
    phases.push(state);
    ...
    for (const s of phases) {
      for (const viewer of s.players) {
        const json = JSON.stringify(redactFor(s, viewer.id));
        for (const other of s.players) {
          if (other.id === viewer.id) continue;
          for (const card of other.hand) {
            // Chosen board cards are public; anything else from a foreign hand is a leak.
  ...
  it('exposes all alive hands once the catch is revealed', () => {
    let state = dealtGame();
    state = reduce(state, { type: 'chooseBoard', playerId: state.starterId, boardCount: 0 });
    ...
    const redacted = redactFor(state, state.players[0].id);
